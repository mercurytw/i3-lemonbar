...
class LemonBar(object):
        ...
        for wsp in self.i3.get_workspaces():
            ...
            if wsp['focused']:
                ...
                wsp_items += "%%{F%s B%s}%s%s%%{F%s B%s T1} %s %%{F%s B%s}%s" % (color_head,
                                                                                 color_wsp, sep_right, wsp_action,
                                                                                 color_back, color_wsp, wsp_name,
                                                                                 color_wsp, color_head, sep_right)
            elif wsp['urgent']:
                wsp_items += "%%{F%s B%s}%s%s%%{F%s B%s T1} %s %%{F%s B%s}%s" % (color_head,
                                                                                 color_mail, sep_right, wsp_action,
                                                                                 color_back, color_mail, wsp_name,
    ...
    def render_focused_title(self):
        return "%%{F%s B%s}%s%%{F%s B%s T2} %s %%{F%s B-}%s%%{F- B- T1} %s" % (color_head,
                                                                               color_sec_b2, sep_right, color_head,
                                                                               color_sec_b2, icon_prog, color_sec_b2,
    ...
    def render_loadavg(self):
        try:
            with open('/proc/loadavg', 'r') as loadfile:
                avg = loadfile.read().split()[0]
        except:
            pass
        color = color_fore if 5.6 > float(avg) else color_mail
        cavg = "%%{F%s }%s%%{F- B%s} %%{T2}%s%%{F%s T1} %s" % (color_sec_b1, sep_left, color_sec_b1, icon_cpu, color, avg)
        return cavg

    def render_datetime(self):
        cdate = "%%{F%s}%s%%{F%s B%s} %%{T2}%s%%{T1} %s" % (color_wsp, sep_left,
                                            color_back, color_wsp, icon_clock,
                                                            time.strftime("%d.%m.%Y"))
